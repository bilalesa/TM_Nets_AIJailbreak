  ...
  if (isSuperAdmin) {
    try {
      ...
      const res = await fetch(
        `${getBackendBaseUrl()}/api/admin/audit?action=daily_wipe&limit=1`,
      ...
      if (res.ok) {
        ...
        if (entry) {
          lastWipe = {
            ...
            admin_email: entry.admin_users?.email ?? null,
          };
        }
      }
    } catch {
      // Non-fatal — last wipe info is cosmetic
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Operational tools for managing the live game state.</p>
      </div>
      <section className="rounded-2xl border border-red-900/60 bg-red-950/20 p-4 sm:p-6">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-red-300">Danger zone</h2>
          <span className="rounded bg-red-950/60 px-2 py-0.5 text-xs uppercase tracking-wide text-red-300">super_admin</span>
        </div>
        <div className="space-y-2 text-sm text-slate-200">
          ...
          <ul className="ml-5 list-disc space-y-1 text-slate-400">
            ...
            <li>Deletes every row in <code>cracked_prompts</code> so the next booth session starts with a fresh anti-cheat hash list.</li>
        ...
        {isSuperAdmin && <div className="mt-5"><WipeButton /></div>}
        {isSuperAdmin && (
            ...
            {lastWipe ? (
              <div className="mt-1 text-sm text-slate-300">
                <div>
                  ...
                  {lastWipe.admin_email && <span className="text-slate-500"> · by {lastWipe.admin_email}</span>}
                </div>
                {lastWipe.details && (
