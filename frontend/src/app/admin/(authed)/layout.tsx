...
export default async function AuthedAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Re-check active flag via backend (avoids direct DB from Vercel).
  try {
    const cookieStore = await cookies();
    const adminToken = cookieStore.get('admin_session_token')?.value;
    const res = await fetch(`${getBackendBaseUrl()}/api/admin/me`, {
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800 bg-slate-900/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex min-w-0 flex-1 items-center gap-4 sm:gap-8">
            <Link href="/admin" className="shrink-0 text-base font-semibold tracking-tight sm:text-lg">
              Admin Console
            </Link>
            <nav className="-mx-1 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto px-1 text-sm [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              {admin.role === 'super_admin' && <NavLink href="/admin/system">System</NavLink>}
            </nav>
          </div>
          <div className="flex w-full items-center justify-between gap-3 text-sm sm:w-auto sm:justify-end">
            ...
            <form action="/api/admin/logout" method="post" className="shrink-0">
              <button type="submit" className="rounded-md border border-slate-700 px-3 py-1.5 text-xs font-medium hover:bg-slate-800">
                Sign out
              </button>
            </form>
          </div>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="rounded-md px-3 py-1.5 text-slate-300 hover:bg-slate-800 hover:text-white">
      {children}
    </Link>
  );
}
